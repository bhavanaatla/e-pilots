...
def uploadDataset():
    ...
    pilot = pd.read_csv("C:/Users/harij/OneDrive/Pilot.csv")
    actuator = pd.read_csv("C:/Users/harij/OneDrive/Actuators.csv")
    physical = pd.read_csv("C:/Users/harij/OneDrive/Physical.csv")
...
def runAP2TD():
    ...
    # Check if model exists, then load it
    if os.path.exists('model/physical_model.json'):
        ...
        lstm_physical.load_weights("model/physical_weights.weights.h5")
    else:
        ...
        lstm_physical.add(Dropout(0.2))
        lstm_physical.add(Dense(2, activation='softmax'))
        
        # Compile and train the model
        lstm_physical.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
        lstm_physical.fit(physical_X_train, physical_y_train, epochs=10, batch_size=32, verbose=1)
    ...
    actual = np.argmax(physical_y_test, axis=1)
    calculateMetrics("AP2TD Physical Features", actual, predict)
...
def runAP2DH():
    ...
    if os.path.exists('model/pilot_model.json'):
        ...
        lstm_pilot.add(Dropout(0.2))
        lstm_pilot.add(Dense(2, activation='softmax'))  # Adjust the output layer according to your number of classes
        lstm_pilot.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    
    # Train the model if it's new
    if not os.path.exists('model/pilot_model.json'):
        lstm_pilot.fit(pilot_X_train, pilot_y_train, epochs=50, batch_size=32, verbose=1)
    ...
    pilot_y_test_classes = np.argmax(pilot_y_test, axis=1)  # Convert to class labels
    calculateMetrics("LSTM Pilot Features", pilot_y_test_classes, predict_classes)
...
def runDH2TD():
    ...
    if os.path.exists('model/pilot_model.json') and os.path.exists('model/pilot_weights.weights.h5'):
        with open('model/pilot_model.json', "r") as json_file:
            loaded_model_json = json_file.read()
            lstm_pilot = model_from_json(loaded_model_json)
        lstm_pilot.load_weights("model/pilot_weights.weights.h5")
        ...
        print("Model weights file not found. Training a new model.")
        lstm_pilot = Sequential()  # defining deep learning sequential object
        # Adding LSTM layer with 100 filters
        lstm_pilot.add(LSTM(100, input_shape=(pilot_X_train.shape[1], pilot_X_train.shape[2])))
...
def graph():
    df = pd.DataFrame([
        ['SVM', 'Sensitivity', sensitivity[0]],
        ['SVM', 'Specificity', specificity[0]],
    ...
    # Correct the pivot method usage with keyword arguments
